...
| ORCE (orchestration engine) | Retained; IR-SI-02's Node-RED-webhook compatibility is satisfied by the shipped example flow (`deployment/helm/charts/orce/flows/contract-target-flow.json`, Workstream G). |
...
- The substitution is documented rather than silently made, so a reviewer
  ...
  discovering an unexplained gap — see Deviation Register #10.
