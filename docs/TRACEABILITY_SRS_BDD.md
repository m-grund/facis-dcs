...
| DCS-NFR-BR-08 | DCS-to-DCS Interoperability Safeguards | ✅ Covered | 17 pack (tagged @NFR-BR-08): authenticated, trusted-peer-only exchanges with audit. Phase 4 infrastructure (not yet wired into the peer-sync flow): validation.RemoteShapeSource/VerifyAgainstOriginatorHub resolves a received document's dcs:schemaRefs back to the ORIGINATOR's public Semantic Hub endpoints and re-validates against those exact shapes rather than the receiver's own local hub — unit-tested (TestVerifyAgainstOriginatorHub, httptest-simulated peer hub) but not yet invoked from the dcstodcs sync handlers, so no @two-instance BDD scenario exercises it end to end. |
