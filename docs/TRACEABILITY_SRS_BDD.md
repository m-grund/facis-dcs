...
| DCS-FR-CWE-13 | Contract Creation | ✅ Covered | 03/contract_creation create-from-template, editable versioned draft. The RED-NOT-RUN bilateral-role scenarios specify deduplicated absolute role URIs, percent-encoded party fragments, originator binding, and rejection of short or unknown roles (`features/03_contract_creation/contract_creation.feature:21`, `features/03_contract_creation/contract_creation.feature:32`); focused backend tests cover URI membership and the bad-request mapping (`backend/internal/contractworkflowengine/command/originatorparty_test.go:12`, `backend/internal/service/contract_create_errors_test.go:13`). |
...
| ID | Requirement | Status | Evidence / disposition |
|---|---|---|---|
| DCS-IR-CWE-01 | Contract Creation UI MUST allow Contract Creators to create and sub… | 🟡 Partial | API: 03 create from approved template. Focused participant-dialog coverage verifies labeled URI options, no free-text or preselected role, and the exactly-two-role gate (`frontend/ClientApp/src/components/participant-selection-dialog.test.ts:70`, `frontend/ClientApp/src/components/participant-selection-dialog.test.ts:96`); submission UI remains out of the harness. |
| DCS-IR-CWE-02 | Contract Creation UI MUST enable population of contract data, inclu… | ✅ Covered | API: parties/policies/evidence populated at create (03, 18, 05 evidence). UI: e2e/contract-fill.spec.ts fills a placeholder through the edit UI and the emitted document carries the forField-bound typed value in an odrl:Offer, with no editor-internal keys leaking. Focused role coverage derives the deduplicated union of percent-decoded role URIs from both rule directions and submits the URI behind its display label (`frontend/ClientApp/src/components/participant-selection-dialog.test.ts:38`, `frontend/ClientApp/src/components/participant-selection-dialog.test.ts:70`); product-level binding and short/unknown-role rejection are specified RED-NOT-RUN in 03/contract_creation (`features/03_contract_creation/contract_creation.feature:21`, `features/03_contract_creation/contract_creation.feature:32`). |
...
| UC-09 | DCS Administration | Roles are derived from credential presentation and then embedded in the ORY/Hydra session (01_authentication_authorization: federated login, role enforcement); negatives suite-wide |
