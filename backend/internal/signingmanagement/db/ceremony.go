...
// present a PoA (via EUDIPLO/OID4VP) that must complete before a PAdES
...
type CeremonyRepo interface {
	...
	MarkCeremonyVerified(ctx context.Context, tx *sqlx.Tx, id, signerDID, vpToken string, poaClaims []byte, kbSdHash string) error
